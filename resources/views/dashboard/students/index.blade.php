        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">اضافة مستخدم</h4>
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('dashboard.get_all_students') }}",
            columns: [
  ...
  {{-- change status --}}
    <script>
        ...
        $(document).on('change','#cousome_switch', function (e) {
                ...
                $.ajax({
                    type: "post",
                    dataType: "json",
                    url: '{{ route('dashboard.is_view_students') }}',
        ...
        $(document).on('click', '#add_user', function (e) {
            ...
            var formData = new FormData($('#add_user_form')[0]);
                $.ajax({
                    ...
                    url: "{{route('dashboard.store_students')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
    ...
    <script>
        $('#edit_user').on('show.bs.modal', function(event) {
            ...
            $(".modal-body #image2").attr('src', "{{  env('APP_URL') }}/attachments/students/"+image);
        ...
        function do_update(){
            ...
            var formData = new FormData($('#edit_user_form')[0]);
                $.ajax({
                    type: 'post',
                    enctype: 'multipart/form-data',
                    url: "{{route('dashboard.update_students')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
   ...
   <script>
        function do_delete(){
            ...
            $.ajax({
                type: 'post',
                url: "{{route('dashboard.destroy_students')}}",
                data: formData,
                processData: false,
                contentType: false,
                success: function (data) {
